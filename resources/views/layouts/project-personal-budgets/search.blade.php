@component('components.accordions.accordion')
    <form action="{{ route('projects.budgets.personal') }}" class="form-horizontal" method="get">
        ...
        @component('components.elements.form.select', ['name' => 'project_id'])
            ...
            @foreach($projectsSelect as $project)
                <option value="{{ $project->id }}" data-subtext="{{ $project->customer ? $project->customer->name : '' }}" @if((int)$projectId === (int)$project->id) selected @endif>{{ '#'.$project->id.' '.$project->name }}</option>
            @endforeach
        @endcomponent
